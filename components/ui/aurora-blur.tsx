...
export interface AuroraBlurProps extends React.ComponentProps<"div"> {
  ...
  saturation?: number;
  opacity?: number;
  showWindowShadow?: boolean;
  windowShadowOpacity?: number;
}

// Light editorial custom pastel palette derived from Morning Glow / Soft Bloom / Deep Horizon
export const CUSTOM_AURORA_LAYERS: AuroraLayer[] = [
  { color: "#FFB5D8", speed: 0.25, intensity: 0.65 }, // Soft Pink
  { color: "#FFD1A0", speed: 0.18, intensity: 0.60 }, // Warm Peach
  { color: "#A0C4FF", speed: 0.14, intensity: 0.55 }, // Periwinkle / Soft Sky Blue
  { color: "#C3C6E8", speed: 0.09, intensity: 0.45 }, // Lilac Lavender Brand Accent
];

export const CUSTOM_SKY_LAYERS: SkyLayer[] = [
  { color: "#F8F9FA", blend: 0.65 }, // Off-white canvas base
  { color: "#FAF8F5", blend: 0.85 }, // Subtle warm light tone
];

const DEFAULT_LAYERS: AuroraLayer[] = CUSTOM_AURORA_LAYERS;
const DEFAULT_SKY_LAYERS: SkyLayer[] = CUSTOM_SKY_LAYERS;
...
function Scene({
  ...
  const uniforms = React.useMemo(
    () => ({
      ...
      u_speed: { value: 1 },
      u_layer1Color: { value: new THREE.Vector3(0.99, 0.71, 0.85) },
      u_layer1Speed: { value: 0.25 },
      u_layer1Intensity: { value: 0.65 },
      u_layer2Color: { value: new THREE.Vector3(0.99, 0.82, 0.63) },
      u_layer2Speed: { value: 0.18 },
      u_layer2Intensity: { value: 0.60 },
      u_layer3Color: { value: new THREE.Vector3(0.63, 0.77, 1.0) },
      u_layer3Speed: { value: 0.14 },
      u_layer3Intensity: { value: 0.55 },
      u_layer4Color: { value: new THREE.Vector3(0.76, 0.78, 0.91) },
      u_layer4Speed: { value: 0.09 },
      u_layer4Intensity: { value: 0.45 },
      u_noiseScale: { value: 2.8 },
      u_movementX: { value: -1.0 },
      u_movementY: { value: -1.8 },
      u_verticalFade: { value: 0.4 },
      u_bloomIntensity: { value: 1.4 },
      u_skyColor1: { value: new THREE.Vector3(0.97, 0.98, 0.98) },
      u_skyColor2: { value: new THREE.Vector3(0.98, 0.97, 0.96) },
      u_skyBlend1: { value: 0.85 },
      u_skyBlend2: { value: 0.65 },
      u_brightness: { value: 0.95 },
      u_saturation: { value: 1.05 },
...
export function AuroraBlur({
  ...
  return (
    ...
    >
      ...
      {/* Atmospheric Window Blinds Shadow Texture (asset 20.jpeg) */}
      {showWindowShadow && (
        <div
          className="absolute inset-0 pointer-events-none z-[2] mix-blend-multiply transition-opacity duration-700"
          style={{
      ...
      {/* Soft Vignette Edge Softener */}
      <div
        className="absolute inset-0 pointer-events-none z-[3] bg-[radial-gradient(ellipse_90%_75%_at_50%_50%,transparent_50%,var(--bg-light)_100%)]"
        aria-hidden="true"
      />
